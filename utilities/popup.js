...
$( document ).on( "click", "#bt_options", function() {
  openOptions();
  event.preventDefault();
});
...
if (typeof cur_volume !== "undefined" && cur_volume !== ""){
	...
	document.getElementById('vol_range').value =cur_volume*10;
}
document.getElementById('vol_range').onchange=function(e){
  ...
  console.log("Value = "+this.value/10);
  chrome.storage.sync.set({'valuevol': this.value/10}, function() {
...
if(voloff.checked == true){
	document.getElementById('carr_range').innerHTML = 1;
	document.getElementById('vol_range').value = 1;
